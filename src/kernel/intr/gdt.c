...
typedef struct {
    ...
    uint8_t  privl      :  2; // privilege (ring level)
...
void gdt_init() {
    ...
    //gdt_init_entry(5, (unsigned int) tss, sizeof (tss)); // pr=1 privl=3 ex=1 ac=1 sz=1
    gdt[0].ac = 0; gdt[0].rw = 0; gdt[0].dc = 0; gdt[0].ex = 0; gdt[0].dt = 0; gdt[0].privl = 0; gdt[0].pr = 0; gdt[0].sz = 0; gdt[0].gr = 0;
    gdt[1].ac = 0; gdt[1].rw = 1; gdt[1].dc = 0; gdt[1].ex = 1; gdt[1].dt = 1; gdt[1].privl = 0; gdt[1].pr = 1; gdt[1].sz = 1; gdt[1].gr = 1;
    gdt[2].ac = 0; gdt[2].rw = 1; gdt[2].dc = 0; gdt[2].ex = 0; gdt[2].dt = 1; gdt[2].privl = 0; gdt[2].pr = 1; gdt[2].sz = 1; gdt[2].gr = 1;
    gdt[3].ac = 0; gdt[3].rw = 1; gdt[3].dc = 0; gdt[3].ex = 1; gdt[3].dt = 1; gdt[3].privl = 3; gdt[3].pr = 1; gdt[3].sz = 1; gdt[3].gr = 1;
    gdt[4].ac = 0; gdt[4].rw = 1; gdt[4].dc = 0; gdt[4].ex = 0; gdt[4].dt = 1; gdt[4].privl = 3; gdt[4].pr = 1; gdt[4].sz = 1; gdt[4].gr = 1;
    gdt_load();
    gdt_flush();
}